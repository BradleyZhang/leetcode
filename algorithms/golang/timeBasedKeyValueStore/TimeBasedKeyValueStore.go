...
type TimeMap struct {
	key2t map[string][]int
	key2v map[string][]string
}

func Constructor() TimeMap {
	return TimeMap{
		key2t: make(map[string][]int),
		key2v: make(map[string][]string),
	}
}

func (this *TimeMap) Set(key string, value string, timestamp int) {
	this.key2t[key] = append(this.key2t[key], timestamp)
	this.key2v[key] = append(this.key2v[key], value)
}
...
func (this *TimeMap) Get(key string, timestamp int) string {
	...
	for left <= right {
		...
		if timestamp == t[mid] {
			return this.key2v[key][mid]
		} else if timestamp < t[mid] {
			right = mid - 1
		} else {
			i = mid
			left = mid + 1

...
// 因为board长宽为常数 时间复杂度为 O(1)
func isValidSudoku(board [][]byte) bool {
	colMaps := make([]map[int]bool, 9)
	rowMaps := make([]map[int]bool, 9)
	boxMaps := make([]map[int]bool, 9)
	for i := 0; i < 9; i++ {
		colMaps[i] = make(map[int]bool)
		rowMaps[i] = make(map[int]bool)
		boxMaps[i] = make(map[int]bool)
	}
	...
	for i, rows := range board {
		for j, num := range rows {
			...
			if rowMaps[i][digit] ||
				colMaps[j][digit] ||
				boxMaps[getBoxIndex(i, j)][digit] {
				return false
			}

			rowMaps[i][digit] = true
			colMaps[j][digit] = true
			boxMaps[getBoxIndex(i, j)][digit] = true
